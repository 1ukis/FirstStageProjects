...
const books = [
  ...
  },
  {
    ...
    pages: 140,
    difficulty: "Easy",
    description: "A classic satirical fable warning against totalitarian corruption of ideals.",
...
let state = {
  ...
  completedBooks: [],     // Array of book IDs
  simulatedDate: new Date("2026-06-12") // Root date requested by user
};

// ==========================================================================
// SVG COVER DRAWING ENGINE
// ==========================================================================
...
function renderBookCoverSvg(book) {
  return `
    <svg class="cover-illustration" viewBox="0 0 100 100" preserveAspectRatio="none">
      ...
      <!-- Minimalist spine text simulation -->
...
// Helper to inject inline linearGradient definitions to a shared SVG defs element
...
// Append SVG defs container to DOM so covers can reference gradients correctly
...
function updateDashboardMetrics() {
  const totalBooksCount = books.length + companionBooks.length; // 37 + 3 = 40
  ...
  // Daily Stoic countdown or status
  ...
  // active focus month based on date
  ...
  // Pace calculator
  let pace = 0;
  if (daysLeft > 0 && remainingPages > 0) {
    pace = (remainingPages / daysLeft).toFixed(1);
  }
  
  // Progress percent
  const progressPercent = Math.round((totalCompletedPages / totalPages) * 100);
  ...
  document.getElementById("daily-pace").textContent = pace;
  document.getElementById("days-remaining").innerHTML = `<strong>${daysLeft}</strong> Days Remaining`;
...
function renderCurriculumList() {
  ...
  if (state.currentView === "timeline") {
    ...
    months.forEach(monthNum => {
      const monthBooks = books.filter(b => b.month === monthNum);
      if (monthBooks.length === 0) return;
      
      // Is this month the active month?
      const oneMonthMs = (CURRICULUM_END_DATE.getTime() - CURRICULUM_START_DATE.getTime()) / 12;
      ...
      const isActiveMonth = currentActiveMonth === monthNum;
      
      const groupEl = document.createElement("div");
      groupEl.className = "timeline-group";
      groupEl.id = `month-group-${monthNum}`;
      
      // Filter logic
      if (state.activeFilter !== "all" && parseInt(state.activeFilter) !== monthNum) {
        groupEl.style.display = "none";
      }
      
      const labelData = monthBooks[0].monthLabel;
      ...
      groupEl.innerHTML = `
        <div class="timeline-group-header">
          ...
          <span class="group-date">${labelData.split(' (')[1].replace(')', '')}</span>
        </div>
        <div class="books-grid">
          ${monthBooks.map(book => renderBookCard(book)).join("")}
        </div>
      `;
  ...
  return `
    <article class="book-card ${isCompleted ? 'completed' : ''}" id="card-${book.id}">
      ...
      <div class="book-details">
        <span class="theme-tag ${book.themeClass}">${book.theme}</span>
        ...
        <p class="book-summary" style="margin-top:-10px; border-top:1px dashed rgba(255,255,255,0.06); padding-top:10px; color:#cbd5e1; font-style:italic; font-size:0.75rem;">
          <strong>Focus:</strong> ${book.focusQuestion}
        </p>
...
// ==========================================================================
// NAVIGATION & FILTER BUBBLES GENERATION
// ==========================================================================
function renderFilters() {
  ...
  if (state.currentView === "timeline") {
    // Generate Month 1 to 12 buttons
    for (let m = 1; m <= 12; m++) {
      const chip = document.createElement("button");
      chip.className = `filter-chip ${state.activeFilter === String(m) ? 'active' : ''}`;
      chip.textContent = `Month ${m}`;
      chip.addEventListener("click", () => selectFilter(String(m)));
      container.appendChild(chip);
    }
  } else {
    // Generate Theme chips
    const themes = [...new Set(books.map(b => b.theme))];
...
function selectFilter(filterValue) {
  ...
  // Re-render curriculum list showing filtered elements
...
// View Switches
document.getElementById("btn-timeline-view").addEventListener("click", (e) => {
...
document.getElementById("btn-theme-view").addEventListener("click", (e) => {
...
// ==========================================================================
// SIMULATED DATE WIDGET (OUTSTANDING EXTRA VALUE FOR THE TIMELINE INTEGRITY)
// ==========================================================================
function setupDateWidget() {
  // Turn date display into an interactive date input!
  ...
  dateInput.addEventListener("change", (e) => {
    ...
    // Recalculate dashboard metrics and redraw active badges
    updateDashboardMetrics();
    renderCurriculumList();
  });
}
...
function loadProgress() {
  ...
  if (saved) {
    try {
      state.completedBooks = JSON.parse(saved);
    } catch(e) {
